...
// Offset so the bg ring emits from the estimated SP center when the logo shimmer peaks.
const PHASE_OFFSET = 0.29
const LOGO_GAP = 1
const LOGO_TOP_BIAS = -1
const LOGO_LEFT_WIDTH = mark.left[0]?.length ?? 0
const LOGO_LINES = mark.left.map((line, index) => line + " ".repeat(LOGO_GAP) + mark.right[index])
...
export class GoUpsellArtPainter {
  ...
  setShowLogo(value: boolean) {
    if (this.showLogo === value) return false
    this.showLogo = value
    this.invalidateCache()
    return true
  }
  ...
  render(frameBuffer: OptimizedBuffer, options: GoUpsellArtRenderOptions = {}) {
    ...
    if (this.showLogo) this.drawLogo(frameBuffer, this.elapsed, rgb)
  }
  ...
  private buildFrameCache(frameBuffer: OptimizedBuffer, rgb: boolean) {
    ...
    for (; this.cacheBuildIndex < end; this.cacheBuildIndex++) {
      ...
      if (this.showLogo) this.drawLogo(frameBuffer, t, rgb)
  ...
  private rebuildCellTemplate(frameBuffer: OptimizedBuffer, rgb: boolean) {
    ...
    if (!this.showLogo || this.geometryWidth < LOGO_WIDTH || this.geometryHeight < LOGO_HEIGHT) {
