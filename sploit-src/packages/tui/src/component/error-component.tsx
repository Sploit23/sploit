...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // Safe fallback palette per mode (mirrors theme/assets/default.json) since the
      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>
              sploit {InstallationVersion} <b style={{ fg: "#ff4444" }}>by Flávio Alex</b>
            </text>
          </box>
        </Show>
      </box>
...
function buildIssueURL(message: string, stack: string) {
  const url = new URL("https://github.com/Sploit23/sploit/issues/new")
  url.searchParams.set("title", `TUI crash: ${message}`)
  url.searchParams.set("sploit-version", InstallationVersion)
  url.searchParams.set("os", describeOS())
  url.searchParams.set("terminal", describeTerminal())
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the sploit crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The sploit TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
