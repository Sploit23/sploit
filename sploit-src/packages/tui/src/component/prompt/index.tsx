...
import { readImageParts } from "../../prompt/image-path"
import { usePromptStash } from "../../prompt/stash"
import { DialogStash } from "../dialog-stash"
import { type AutocompleteRef, Autocomplete } from "./autocomplete"
import { useRenderer, useTerminalDimensions, type JSX } from "@opentui/solid"
import type { AssistantMessage, FilePart, UserMessage } from "@sploit-ai/sdk/v2"
...
export type PromptProps = {
  ...
  placeholders?: {
    normal?: (string | { en: string; pt: string })[]
    shell?: (string | { en: string; pt: string })[]
  }
}
...
export function Prompt(props: PromptProps) {
  ...
  function promptModelWarning() {
    toast.show({
      variant: "warning",
      message: "Conecte um provedor para enviar mensagens",
      duration: 3000,
    })
        ...
        run: async () => {
          ...
          if (!content || !content.mime.startsWith("image/")) {
            toast.show({
              ...
              message: "Copy a screenshot first, then try again.",
              variant: "info",
            })
            return
          }
  ...
  useBindings(() => {
    return {
      ...
      bindings: [
        ...
        ...tuiConfig.keybinds.get("prompt.paste.image"),
      ],
    }
  })
  ...
  async function submitInner() {
    ...
    if (sessionID == null) {
      ...
      if (res.error) {
        ...
        toast.show({
          message: "Falha ao criar sessão. Abra o console para mais detalhes.",
          variant: "error",
        })
    ...
    if (store.mode === "shell") {
      ...
      move.startSubmit()
      const autoImageParts = await readImageParts(inputText, paths.cwd, paths.home, nonTextParts)
      const hasImage = [...nonTextParts, ...autoImageParts].some(
        (part) => "mime" in part && part.mime.startsWith("image/"),
      )
      const modelInfo = sync.data.provider
        .find((item) => item.id === selectedModel.providerID)
        ?.models[selectedModel.modelID]
      if (hasImage && modelInfo && !modelInfo.capabilities.input.image) {
        toast.show({
          title: "Model without vision",
          message: `${modelInfo.name ?? selectedModel.modelID} cannot see images. Switch to a vision-capable model.`,
          variant: "warning",
        })
      }
      sdk.client.session
  ...
  const placeholderText = createMemo(() => {
    ...
    if (store.mode === "shell") {
      return langIndex() === 0 ? "Digite um comando -" : "Execute um comando -"
    }
    return langIndex() === 0 ? "Digite sua mensagem -" : "Digite sua mensagem -"
  })
          ...
          >
            ...
            <box flexDirection="row" flexShrink={0} paddingTop={1} gap={1} justifyContent="space-between">
              ...
              <box flexDirection="row" gap={1} alignItems="center">
                <Show when={props.showAgent !== false && local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <box flexDirection="row" gap={1} alignItems="center">
                      <text fg={fadeColor(local.agent.color(agent().name), agentMetaAlpha())}>-</text>
                      <text fg={fadeColor(theme.text, agentMetaAlpha())}>
