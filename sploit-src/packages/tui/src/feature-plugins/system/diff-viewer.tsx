...
import type { TuiPlugin, TuiPluginApi, TuiRouteCurrent } from "@sploit-ai/plugin/tui"
import type { SnapshotFileDiff, VcsFileDiff } from "@sploit-ai/sdk/v2"
import {
  TextAttributes,
  type BorderSides,
  type BoxRenderable,
...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  useBindings(() => ({
    commands,
    bindings: [
      { key: "j,down", cmd: "diff.down", desc: "Mover visualizador de diff para baixo" },
      { key: "k,up", cmd: "diff.up", desc: "Mover visualizador de diff para cima" },
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: "Rolar visualizador de diff para baixo" },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: "Rolar visualizador de diff para cima" },
      { key: "m", cmd: "diff.mark_reviewed", desc: "Marcar arquivo selecionado como revisado" },
