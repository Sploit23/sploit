...
const SPLOIT_MODE_KEY = "sploit.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(SPLOIT_MODE_KEY, OPENCODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(SPLOIT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(SPLOIT_MODE_KEY, stack.at(-1)?.mode ?? OPENCODE_BASE_MODE)
  }
  ...
  const stackApi = {
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(SPLOIT_MODE_KEY, undefined)
