...
import type { PermissionRequest } from "@sploit-ai/sdk/v2"
...
function EditBody(props: { request: PermissionRequest }) {
  ...
  return (
    <box flexDirection="column" gap={1}>
      ...
      <Show when={!diff()}>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Nenhum diff fornecido</text>
        </box>
      </Show>
    </box>
  )
  ...
  return (
    <Switch>
      <Match when={store.stage === "always"}>
        <Prompt
          title="Sempre permitir"
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"Isto permitirá " + props.request.permission + " até o Sploit ser reiniciado."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>Isto permitirá os seguintes padrões até o Sploit ser reiniciado</text>
                  <box>
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "list") {
              ...
              return {
                ...
                body: (
                  <Show when={dir}>
                    <box paddingLeft={1}>
                      <text fg={theme.textMuted}>{"Caminho: " + pathFormatter.format(dir)}</text>
                    </box>
                  </Show>
                ),
              }
            ...
            if (permission === "task") {
              ...
              return {
                icon: "#",
                title: `${Locale.titlecase(type)} Tarefa`,
                body: (
                  <Show when={desc}>
                    <box paddingLeft={1}>
              ...
              return {
                ...
                body: (
                  <Show when={patterns.length > 0}>
                    <box paddingLeft={1} gap={1}>
                      <text fg={theme.textMuted}>Padrões</text>
                      <box>
            ...
            if (permission === "doom_loop") {
              return {
                icon: "⟳",
                title: "Continuar após falhas repetidas",
                body: (
                  <box paddingLeft={1}>
                    <text fg={theme.textMuted}>Isto mantém a sessão rodando apesar de falhas repetidas.</text>
                  </box>
                ),
              }
            }
            ...
            return {
              icon: "⚙",
              title: `Chamar ferramenta ${permission}`,
              body: (
                <box paddingLeft={1}>
                  <text fg={theme.textMuted}>{"Ferramenta: " + permission}</text>
                </box>
              ),
            }
          }
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              <box flexDirection="row" gap={1} flexShrink={0}>
                ...
                <text fg={theme.text}>Permissão necessária</text>
              </box>
          ...
          const body = (
            <Prompt
              ...
              header={header()}
              body={current.body}
              options={{ once: "Permitir uma vez", always: "Permitir sempre", reject: "Rejeitar" }}
              escapeKey="reject"
              fullscreen
              onSelect={(option) => {
                if (option === "always") {
...
function RejectPrompt(props: { onConfirm: (message: string) => void; onCancel: () => void }) {
  ...
  useBindings(() => ({
    ...
    commands: [
      {
        name: "app.exit",
        title: "Cancelar rejeição de permissão",
        category: "Permissão",
        run() {
          props.onCancel()
        },
    ...
    bindings: [
      { key: "escape", desc: "Cancelar rejeição de permissão", group: "Permissão", cmd: () => props.onCancel() },
      ...tuiConfig.keybinds.get("app.exit"),
      {
        key: "return",
        desc: "Confirmar rejeição de permissão",
        group: "Permissão",
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        <box flexDirection="row" gap={1} paddingLeft={1}>
          ...
          <text fg={theme.text}>Rejeitar permissão</text>
        </box>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Diga ao Sploit o que fazer de diferente</text>
        </box>
      </box>
      <box
  ...
  useBindings(() => ({
    ...
    commands: [
      ...
      },
      {
        ...
        title: "Alternar permissão em tela cheia",
        category: "Permissão",
        run() {
    ...
    bindings: [
      {
        key: "left",
        desc: "Opção de permissão anterior",
        group: "Permissão",
        cmd: () => {
      ...
      {
        key: "right",
        desc: "Próxima opção de permissão",
        group: "Permissão",
        cmd: () => {
          const idx = keys.indexOf(store.selected)
      ...
      {
        key: "return",
        desc: "Selecionar opção de permissão",
        group: "Permissão",
