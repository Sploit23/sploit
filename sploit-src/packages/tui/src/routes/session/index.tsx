...
import { SquadSetupBanner } from "./squad-setup.tsx"
...
export function Session() {
  ...
  // Auto-submete o prompt injetado por --prompt (ex.: relaunch do ciclo de
  // auto-melhoria) assim que sync e modelo estiverem prontos, como a Home faz.
  createEffect(() => {
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!selectedModel) {
          toast.show({
            variant: "warning",
            message: "Conecte um provedor para resumir esta sessão",
            duration: 3000,
          })
          return
        }
    ...
    {
      title: "Desfazer mensagem anterior",
      value: "session.undo",
      category: "Sessão",
      slash: {
        name: "undo",
    ...
    },
    {
      title: (() => {
        ...
        if (next === "hide") return "Recolher raciocínio"
        return "Expandir raciocínio"
      })(),
    ...
    {
      title: showDetails() ? "Ocultar detalhes da ferramenta" : "Mostrar detalhes da ferramenta",
    ...
    {
      title: "Alternar barra de rolagem da sessão",
    ...
    {
      title: showGenericToolOutput() ? "Ocultar saída genérica de ferramenta" : "Mostrar saída genérica de ferramenta",
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!lastAssistantMessage) {
          toast.show({ message: "Nenhuma mensagem do assistente encontrada", variant: "error" })
          dialog.clear()
          return
        }
          ...
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
            <Show when={session()}>
              ...
              <box flexShrink={0}>
                ...
                <Show when={visible()}>
                  <box flexShrink={0} flexDirection="row" justifyContent="flex-end" alignItems="center" gap={1}>
                    <Show when={local.agent.current()}>
                      {(agent) => (
                        <box flexDirection="row" gap={1} alignItems="center">
                          <text fg={local.agent.color(agent().name)}>-</text>
                          <text fg={theme.text}>{Locale.titlecase(agent().name)}</text>
                        </box>
                      )}
                    </Show>
                  </box>
      ...
      <Match when={stringValue(props.metadata.output) !== undefined}>
        ...
        >
          <box gap={1}>
            ...
            <Show when={collapsed().overflow}>
              <text fg={theme.textMuted}>{expanded() ? "Clique para recolher" : "Clique para expandir"}</text>
            </Show>
          </box>
        </BlockTool>
      </Match>
      <Match when={true}>
        <InlineTool icon="$" pending="Executando comando..." complete={stringValue(props.input.command)} part={props.part}>
  ...
  return (
    <Switch>
      <Match when={props.metadata.diagnostics !== undefined}>
        <BlockTool title={"# Gravado " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
      ...
      <Match when={true}>
        <InlineTool
          icon="←"
          pending="Preparando gravação..."
          complete={stringValue(props.input.filePath)}
          part={props.part}
        >
          Gravou {pathFormatter.format(stringValue(props.input.filePath))}
        </InlineTool>
      </Match>
    </Switch>
  )
...
function Glob(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="Procurando arquivos..." complete={stringValue(props.input.pattern)} part={props.part}>
      Glob "{stringValue(props.input.pattern)}"{" "}
      <Show when={stringValue(props.input.path)}>em {pathFormatter.format(stringValue(props.input.path))} </Show>
      <Show when={numberValue(props.metadata.count)}>
        ({numberValue(props.metadata.count)} {numberValue(props.metadata.count) === 1 ? "resultado" : "resultados"})
      </Show>
    </InlineTool>
  )
}
...
function Read(props: ToolProps) {
  ...
  return (
    <>
      <InlineTool
        icon="→"
        pending="Lendo arquivo..."
        complete={stringValue(props.input.filePath)}
        spinner={isRunning()}
        part={props.part}
      >
        Leitura {pathFormatter.format(stringValue(props.input.filePath))} {input(props.input, ["filePath"])}
      </InlineTool>
      <For each={loaded()}>
        {(filepath) => (
          <box paddingLeft={3}>
            <text paddingLeft={3} fg={theme.textMuted}>
              ↳ Carregado {pathFormatter.format(filepath)}
            </text>
          </box>
        )}
      </For>
...
function Grep(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending="Procurando conteúdo..." complete={stringValue(props.input.pattern)} part={props.part}>
      Grep "{stringValue(props.input.pattern)}"{" "}
      <Show when={stringValue(props.input.path)}>em {pathFormatter.format(stringValue(props.input.path))} </Show>
      <Show when={numberValue(props.metadata.matches)}>
        ({numberValue(props.metadata.matches)} {numberValue(props.metadata.matches) === 1 ? "resultado" : "resultados"})
      </Show>
    </InlineTool>
  )
}
...
function WebFetch(props: ToolProps) {
  return (
    <InlineTool icon="%" pending="Buscando na web..." complete={stringValue(props.input.url)} part={props.part}>
...
function WebSearch(props: ToolProps) {
  return (
    <InlineTool icon="◈" pending="Pesquisando na web..." complete={stringValue(props.input.query)} part={props.part}>
      {webSearchProviderLabel(props.metadata.provider)} "{stringValue(props.input.query)}"{" "}
      <Show when={numberValue(props.metadata.numResults)}>({numberValue(props.metadata.numResults)} resultados)</Show>
    </InlineTool>
  )
}
  ...
  return (
    <InlineTool
      ...
      onClick={() => {
        ...
        const status = retry()
        if (status) void DialogAlert.show(dialog, "Erro de nova tentativa", status.message)
      }}
    >
      {content()}
    </InlineTool>
...
export function formatSubagentToolcalls(count: number) {
  return `${count} chamada${count === 1 ? "" : "s"} de ferramenta`
}

export function formatSubagentTitle(agent: string, description: string, background: boolean) {
  return `${agent} Tarefa${background ? " (em segundo plano)" : ""} — ${description}`
}

export function formatSubagentRetry(attempt: number, message: string) {
  return `Tentando novamente (tentativa ${attempt}) · ${message}`
}
...
function Execute(props: ToolProps) {
  ...
  const content = createMemo(() => {
    ...
    for (const call of calls()) {
      ...
      lines.push(`↳ ${call.tool}${args ? ` ${args}` : ""}${call.status === "error" ? " (falhou)" : ""}`)
    }
    return lines.join("\n")
  })
  ...
  return (
    <Switch>
      <Match when={stringValue(props.metadata.diff) !== undefined}>
        <BlockTool title={"← Edição " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
          <box paddingLeft={1}>
            <diff
              diff={diffContent()}
              view={view()}
      ...
      <Match when={true}>
        <InlineTool icon="←" pending="Preparando edição..." complete={stringValue(props.input.filePath)} part={props.part}>
          Edição {pathFormatter.format(stringValue(props.input.filePath))} {input({ replaceAll: props.input.replaceAll })}
        </InlineTool>
      </Match>
    </Switch>
  )
...
function ApplyPatch(props: ToolProps) {
  ...
  function title(file: { type: string; relativePath: string; filePath: string; deletions: number }) {
    if (file.type === "delete") return "# Removido " + file.relativePath
    if (file.type === "add") return "# Criado " + file.relativePath
    if (file.type === "move") return "# Movido " + pathFormatter.format(file.filePath) + " → " + file.relativePath
    return "← Patch em " + file.relativePath
  }
  ...
  return (
    <Switch>
              ...
              <Show
                ...
                fallback={
                  <text fg={theme.diffRemoved}>
                    -{file.deletions} linha{file.deletions !== 1 ? "s" : ""}
                  </text>
                }
              >
      ...
      <Match when={true}>
        <InlineTool icon="%" pending="Preparando patch..." failure="Falha no patch" complete={false} part={props.part}>
          Patch
        </InlineTool>
      </Match>
    </Switch>
  ...
  return (
    <Switch>
      <Match when={parseTodos(props.metadata.todos).length}>
        <BlockTool title="# Tarefas" part={props.part}>
          <box>
      ...
      <Match when={true}>
        <InlineTool
          icon="⚙"
          pending="Atualizando tarefas..."
          failure="Falha ao atualizar tarefas"
          complete={false}
          part={props.part}
        >
          Atualizando tarefas...
...
function Question(props: ToolProps) {
  ...
  function format(answer?: ReadonlyArray<string>) {
    if (!answer?.length) return "(sem resposta)"
    return answer.join(", ")
  }
  ...
  return (
    <Switch>
      <Match when={answers()}>
        <BlockTool title="# Perguntas" part={props.part}>
          <box gap={1}>
            <For each={questions()}>
              {(q, i) => (
      ...
      <Match when={true}>
        <InlineTool icon="→" pending="Fazendo perguntas..." complete={count()} part={props.part}>
          Fez {count()} pergunta{count() !== 1 ? "s" : ""}
        </InlineTool>
      </Match>
    </Switch>
  )
...
function Skill(props: ToolProps) {
  return (
    <InlineTool icon="→" pending="Carregando skill..." complete={stringValue(props.input.name)} part={props.part}>
