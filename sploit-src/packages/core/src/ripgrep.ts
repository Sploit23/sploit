...
import { Entry, Match } from "@sploit-ai/schema/filesystem"
...
export class Service extends Context.Service<Service, Interface>()("@sploit/v2/Ripgrep") {}
      ...
      return abortable.pipe(
        Effect.mapError((cause) =>
          cause instanceof Error || cause instanceof InvalidPatternError
            ? cause
            : failure(`ripgrep execution failed: ${messageOf(cause)}`, cause),
        ),
      )
    }
