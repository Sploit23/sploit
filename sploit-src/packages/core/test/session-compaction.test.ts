...
test("compaction prompt includes graph anchors when provided", () => {
  const prompt = SessionCompaction.buildPrompt({
    context: ["conversation history"],
    anchors: "- sploit-src/packages/core/src/session/compaction.ts",
  })

  expect(prompt).toContain("Project anchors")
  expect(prompt).toContain("sploit-src/packages/core/src/session/compaction.ts")
})
