...
import { CloudflareWorkersAIPlugin } from "@sploit-ai/core/plugin/provider/cloudflare-workers-ai"
  ...
  it.effect("uses env API key over auth or configured API key and keeps the Cloudflare User-Agent", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "env-key" }, () =>
      Effect.gen(function* () {
        ...
        expect(headers["user-agent"]).toMatch(/^sploit\/.* cloudflare-workers-ai \(.+\) ai-sdk\/openai-compatible\//)
      }),
    ),
  )
