...
import { define } from "@sploit-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@sploit-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "sploit.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "sploit.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
