...
import { ConfigExternalPlugin } from "@sploit-ai/core/config/plugin/external"
...
import { PluginHost } from "@sploit-ai/core/plugin/host"
...
describe("ConfigExternalPlugin", () => {
  it.live("resolves and loads a configured Promise plugin with options", () =>
    Effect.gen(function* () {
      ...
      const document = path.join(import.meta.dir, "sploit.json")
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Document({
                  type: "document",
                  path: path.join(import.meta.dir, "sploit.json"),
                  info: decode({
                    plugins: [
                      {
