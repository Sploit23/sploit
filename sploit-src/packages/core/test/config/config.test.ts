...
import { Policy } from "@sploit-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads sploit JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "sploit.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "sploit.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "sploit.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "sploit.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "sploit.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "sploit.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .sploit configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".sploit"), { recursive: true })
            await fs.mkdir(path.join(directory, ".sploit"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "sploit.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "sploit.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "sploit.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "sploit.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "sploit.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".sploit", "sploit.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".sploit", "sploit.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".sploit")),
              AbsolutePath.make(path.join(directory, ".sploit")),
            ])
