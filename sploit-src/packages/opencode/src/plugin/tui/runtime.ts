...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@sploit-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@sploit-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@sploit-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@sploit-ai/tui/context/theme"
import { Global } from "@sploit-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@sploit-ai/core/util/flock"
import { Flag } from "@sploit-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@sploit-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@sploit-ai/core/v1/config/plugin"
import { createCommandShim } from "@sploit-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@sploit-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".sploit"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".sploit", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".sploit", "tui.json"),
  }
}
