...
export async function checkPluginCompatibility(target: string, sploitVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(sploitVersion) || semver.major(sploitVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.sploit
  if (typeof range !== "string") return
  if (!semver.satisfies(sploitVersion, range)) {
    throw new Error(`Plugin requires sploit ${range} but running ${sploitVersion}`)
  }
}
