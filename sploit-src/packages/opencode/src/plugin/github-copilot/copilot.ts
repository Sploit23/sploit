import type { Hooks, PluginInput } from "@sploit-ai/plugin"
import type { Model } from "@sploit-ai/sdk/v2"
      ...
      async models(provider, ctx) {
        ...
        return CopilotModels.get(
          base(auth.enterpriseUrl),
          {
            ...
            "User-Agent": `sploit/${InstallationVersion}`,
          ...
          .then((result) => {
            ...
            const pickerModels = entries.filter(([, model]) => result.pickerEnabled.has(model.api.id))
            // Some Copilot plans (e.g. free) report every model with
            // model_picker_enabled=false. Fall back to all usable models so the
            // provider catalog is not left empty.
            return Object.fromEntries(pickerModels.length > 0 ? pickerModels : entries)
          })
          .catch((error) => {
            models = {}
