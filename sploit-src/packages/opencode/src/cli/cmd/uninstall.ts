...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall sploit and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Sploit")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Sploit!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# sploit") || content.includes(".sploit/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".sploit/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".sploit/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".sploit"))
    ) {
      continue
    }
