...
//
// Renders the full sploit entry logo and a compact [P] exit badge, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  const badge = mark.left.map((row, i) => `${row} ${mark.right[i] ?? ""}`)
  let height = 1

  if (kind === "entry") {
    const top = 1
    const body_left = (badge[0]?.length ?? 0) + 2

    for (let i = 0; i < badge.length; i += 1) {
      draw(lines, badge[i] ?? "", {
        left: 0,
        top: top + i,
        fg: input.theme.badge,
        shadow: leftShadow,
      })
    }

    push(lines, body_left, top, "Sploit", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
