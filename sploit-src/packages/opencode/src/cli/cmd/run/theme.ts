...
import type { TuiThemeCurrent } from "@sploit-ai/plugin/tui"
...
function splashTheme(theme: TuiThemeCurrent, indexed: RGBA[]): RunSplashTheme {
  ...
  return {
    ...
    badge: nearestIndexed(indexed, theme.primary),
  }
}
...
export async function resolveRunTheme(renderer: CliRenderer): Promise<RunTheme> {
  try {
    ...
    const shared = await import("@sploit-ai/tui/context/theme")
