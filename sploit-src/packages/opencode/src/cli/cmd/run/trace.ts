...
//
// Enable with SPLOIT_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/sploit/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.SPLOIT_DIRECT_TRACE) {
    state = false
    return undefined
  }
