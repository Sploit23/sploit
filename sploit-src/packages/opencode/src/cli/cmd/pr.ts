...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run sploit",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found sploit session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const importResult = yield* Effect.promise(() =>
            Process.text(["sploit", "import", sessionUrl], { nothrow: true }),
          )
    ...
    UI.println()
    UI.println("Starting sploit...")
    UI.println()

    const sploitArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn(["sploit", ...sploitArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`sploit exited with code ${code}`))
  }),
})
