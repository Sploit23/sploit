import type { PermissionV1 } from "@sploit-ai/core/v1/permission"
import { FSUtil } from "@sploit-ai/core/fs-util"
// CLI entry point for `sploit run` and `sploit --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`sploit --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`sploit --mini --attach`): connects to a running
//      sploit server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@sploit-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run sploit with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running sploit server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'sploit')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function share(_sdk: OpencodeClient, _sessionID: string) {
        // O Sploit é local: sessões não são publicadas em servidor externo.
      }
