...
import { PositiveInt } from "@sploit-ai/core/schema"
...
Usage notes:
  ...
  - Long-running processes: NEVER run a server, watcher, or daemon synchronously in this tool. It blocks until the timeout and gets aborted. Use your project's own script with a detached/background flag (e.g. \`-Detached\`, \`Start-Process\`, \`& ... & \`) or specify a larger timeout and still expect it to block. If the goal is to start a service for later use, launch it in the background and return immediately.
...
Usage notes:
  ...
  - Long-running processes: NEVER run a server, watcher, or daemon synchronously in this tool. It blocks until the timeout and gets aborted. Use \`Start-Process\` or the script's own detached flag (e.g. \`.\scripts\sploit-web.ps1 -Detached\`) to launch services in the background and return immediately.
...
Usage notes:
  ...
  - Long-running processes: NEVER run a server, watcher, or daemon synchronously in this tool. It blocks until the timeout and gets aborted. Use \`start /b\` or the script's own detached flag to launch services in the background and return immediately.
