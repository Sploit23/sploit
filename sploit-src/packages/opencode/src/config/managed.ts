...
const MANAGED_PLIST_DOMAIN = "ai.sploit.managed"

// Keys injected by macOS/MDM into the managed plist that are not Sploit config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/sploit"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "sploit")
    default:
      return "/etc/sploit"
  }
}
