...
import { isRecord } from "@sploit-ai/tui/util/record"
...
import { TuiKeybind } from "@sploit-ai/tui/config/keybind"
...
import { TuiConfig } from "@sploit-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@sploit/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old sploit.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.sploit`
  // folders between cwd and home, and SPLOIT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SPLOIT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SPLOIT_TUI_CONFIG override, if set.
  if (Flag.SPLOIT_TUI_CONFIG) {
    const configFile = Flag.SPLOIT_TUI_CONFIG
  ...
  // 4. `.sploit` directories (and SPLOIT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".sploit") || dir === Flag.SPLOIT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".sploit") && dir !== Flag.SPLOIT_CONFIG_DIR) continue
