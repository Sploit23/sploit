...
import { serviceUse } from "@sploit-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@sploit-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@sploit-ai/core/v1/config/console-state"
import { FSUtil } from "@sploit-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@sploit-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@sploit-ai/core/v1/config/config"
import { RemoteAuthError } from "@sploit-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@sploit-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@sploit/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["sploit.jsonc", "sploit.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SPLOIT_CONFIG && !Flag.SPLOIT_CONFIG_DIR && !Flag.SPLOIT_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "sploit.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "sploit.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "SPLOIT_CONFIG_CONTENT") return "local"
        ...
        if (Flag.SPLOIT_CONFIG) {
          yield* merge(Flag.SPLOIT_CONFIG, yield* loadFile(Flag.SPLOIT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SPLOIT_CONFIG })
        }

        if (!Flag.SPLOIT_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("sploit", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.SPLOIT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SPLOIT_CONFIG_DIR", { path: Flag.SPLOIT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".sploit") || dir === Flag.SPLOIT_CONFIG_DIR) {
            for (const file of ["sploit.json", "sploit.jsonc"]) {
          ...
          // Auto-discovered plugins under `.sploit/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.SPLOIT_CONFIG_CONTENT) {
          const source = "SPLOIT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.SPLOIT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from SPLOIT_CONFIG_CONTENT")
        }
