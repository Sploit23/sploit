...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@sploit-ai/sdk/v2"
...
export class Service extends Context.Service<Service, Interface>()("@sploit/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `sploit auth login` in the terminal",
      name: "Login with sploit",
      id: AuthMethodID,
    }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Sploit service failure", service })
}
