...
import { McpEvent } from "@sploit-ai/schema/mcp-event"
...
function createClient(directory: string) {
  const client = new Client({ name: "sploit", version: InstallationVersion }, CLIENT_OPTIONS)
...
export class Service extends Context.Service<Service, Interface>()("@sploit/MCP") {}
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: sploit mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
