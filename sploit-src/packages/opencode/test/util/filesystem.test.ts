...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "sploit.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "sploit.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sploit.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "sploit.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["sploit.json", "sploit.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "sploit.json"),
        path.join(project, "sploit.jsonc"),
      ])
    })
  })
