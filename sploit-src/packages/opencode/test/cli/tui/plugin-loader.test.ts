...
import type { TuiAttentionSoundPack } from "@sploit-ai/plugin/tui"
...
const { allThemes, addTheme } = await import("@sploit-ai/tui/context/theme")
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".sploit", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".sploit", "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".sploit", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "sploit.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".sploit", "themes", themeFile)
