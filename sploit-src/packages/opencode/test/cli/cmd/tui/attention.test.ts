...
import { createTuiAttention } from "@sploit-ai/tui/attention"
import type { TuiConfig } from "@sploit-ai/tui/config"
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "sploit", message: "focused" }])
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "sploit", message: "hello" }])
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "sploit", message: "hello", sound: false })).toEqual({
      ok: true,
      notification: true,
      sound: false,
    })
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "sploit", message: "hello again" }])
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("sploit.default")
  })
