...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/sploit.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/sploit.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/sploit.jsonc", dir: ".sploit", suggestion: "sploit" },
        expected:
          'Directory ".sploit" in /tmp/sploit.jsonc is not valid. Rename the directory to "sploit" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/sploit.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/sploit.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `sploit models` to list available models",
      "Or check your config (sploit.json) provider/model names",
    ].join("\n")
