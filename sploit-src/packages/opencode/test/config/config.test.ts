...
import { ConfigV1 } from "@sploit-ai/core/v1/config/config"
import { LayerNode } from "@sploit-ai/core/effect/layer-node"
import { httpClient } from "@sploit-ai/core/effect/app-node-platform"
import { Cause, Effect, Exit, Layer, Option } from "effect"
import { NamedError } from "@sploit-ai/core/util/error"
...
import { CrossSpawnSpawner } from "@sploit-ai/core/cross-spawn-spawner"
...
import { ConfigPluginV1 } from "@sploit-ai/core/v1/config/plugin"
...
const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "sploit.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "sploit.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "sploit.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".sploit"), schemaConfig(input.local)) : undefined,
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* FSUtil.use.readFileString(path.join(dir, "sploit.jsonc"))
      expect(content).toContain('"$schema": "https://opencode.ai/config.json"')
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

it.effect("does not create global config when SPLOIT_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
    ...
    yield* withGlobalConfig({}, ({ dir }) =>
      withProcessEnv(
        ...
        Effect.gen(function* () {
          ...
          expect(yield* FSUtil.use.existsSafe(path.join(dir, "sploit.jsonc"))).toBe(false)
        }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
      ),
    )
  }),
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "sploit.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "sploit.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "sploit.jsonc")
...
it.instance("ignores legacy tui keys in sploit config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "sploit.jsonc"),
      `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, "sploit.json"),
      ...
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "sploit.json"))
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "sploit.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .sploit directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .sploit/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .sploit/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .sploit/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only SPLOIT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("SPLOIT_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible SPLOIT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("SPLOIT_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing SPLOIT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable SPLOIT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".sploit", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "sploit.jsonc")
...
it.instance("local .sploit config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://opencode.ai/config.json",
    ...
    yield* FSUtil.use.ensureDir(path.join(test.directory, ".sploit"))
    yield* writeConfigEffect(
      path.join(test.directory, ".sploit"),
      {
        $schema: "https://opencode.ai/config.json",
        mcp: {
          docs: {
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/sploit.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    ...
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/sploit.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/sploit.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/sploit.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/sploit.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/sploit.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/sploit.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/sploit.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
loginPageWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const exit = yield* Config.use.get().pipe(Effect.exit)
      expect(loginPageWellKnown.seen.remote).toBe("https://config.example.com/sploit.json")
      expect(Exit.isFailure(exit)).toBe(true)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "sploit.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.sploit/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.sploit/plugin/demo.ts", "file:///project/.sploit/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.sploit/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".sploit", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("SPLOIT_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .sploit/ directories when flag is set", () =>
    withProcessEnv(
      "SPLOIT_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".sploit", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { SPLOIT_CONFIG_DIR: undefined, SPLOIT_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "SPLOIT_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { SPLOIT_DISABLE_PROJECT_CONFIG: "true", SPLOIT_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
describe("SPLOIT_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in SPLOIT_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "SPLOIT_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in SPLOIT_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.sploit.managed.test",
          PayloadType: "ai.sploit.managed",
