...
describe("tool.grep", () => {
  ...
  it.instance("reports an actionable error when the path does not exist", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const missing = path.join(test.directory, "does-not-exist", "file.ts")
      const info = yield* GrepTool
      const grep = yield* info.init()
      ...
      expect(cause instanceof Error ? cause.message : String(cause)).toContain("Path not found")
    }),
  )
