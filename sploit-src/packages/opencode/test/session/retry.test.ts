...
import type { NamedError } from "@sploit-ai/core/util/error"
...
describe("session.retry.delay", () => {
  ...
  test("uses retryDelay from google.rpc.RetryInfo in the response body", () => {
    ...
    expect(SessionRetry.delay(1, error)).toBe(29471)
  })

  test("uses the retry delay text from the response body", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message: "Quota exceeded",
        isRetryable: true,
        responseBody:
          "Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests.\nPlease retry in 49.205264186s.",
      }).toObject(),
    )
    expect(SessionRetry.delay(1, error)).toBe(49206)
  })

  test("prefers header retry-after over the response body retry delay", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message: "Quota exceeded",
        isRetryable: true,
        ...
        responseBody: JSON.stringify({
          error: {
            code: 429,
            message: "Please retry in 50s.",
            details: [{ "@type": "type.googleapis.com/google.rpc.RetryInfo", retryDelay: "50s" }],
          },
        }),
      }).toObject(),
    )
  ...
  test("falls back to exponential backoff when the body has no retry delay", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message: "Quota exceeded",
        isRetryable: true,
...
describe("session.retry.retryable", () => {
  ...
  test("retries Copilot utility model rate limit even when isRetryable is false", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message:
          "AI_APICallError: Sorry, you've exceeded your rate limit for utility models. Please try again later.",
        isRetryable: false,
        statusCode: 403,
      }).toObject(),
    )
  ...
  test("retries Copilot rate limit text found in the response body", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
        message: "API Error",
        isRetryable: false,
        statusCode: 403,
        responseBody: "Sorry, you've exceeded your rate limit for utility models.",
      }).toObject(),
    )

    expect(SessionRetry.retryable(error, retryProvider)).toEqual({ message: "API Error" })
  })
