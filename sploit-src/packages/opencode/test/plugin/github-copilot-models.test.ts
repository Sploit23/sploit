...
test("falls back to all usable models when no picker models are enabled", async () => {
  globalThis.fetch = mock(() =>
    Promise.resolve(
      new Response(
        JSON.stringify({
  ...
  expect(models["gpt-4.1"].api.id).toBe("gpt-4.1")
  expect(models["gpt-4.1"].api.npm).toBe("@ai-sdk/github-copilot")
})
