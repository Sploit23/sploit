...
//   - SPLOIT_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - SPLOIT_DISABLE_PROJECT_CONFIG : skip walking up for sploit.json
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    SPLOIT_CONFIG_CONTENT: configJson,
    SPLOIT_DISABLE_PROJECT_CONFIG: "1",
    OPENCODE_PURE: "1",
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          SPLOIT_CONFIG_CONTENT: JSON.stringify({
