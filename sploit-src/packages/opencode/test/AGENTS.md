...
- `config?: Partial<Config.Info>` - Write a `sploit.json` config file
...
- Directories are created in the system temp folder with prefix `sploit-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("sploit-test-")
  }),
)
```
